...
When an ADT connection is available, the tests can be run rather than only written:

| Step | Tool |
| --- | --- |
| Read the class and its test include | `readAbapObject` - `includes.testclasses` is the URL, then `getObjectSource` |
| Add a test include to a class that has none | `createTestInclude` |
| Run the tests | `unitTestRun` |
| Drill into a failed class | `unitTestEvaluation` - pass the class object from `unitTestRun` |
| Coverage markers in the editor | `unitTestOccurrenceMarkers` |
| Check the test code compiles before writing it | `syntaxCheckCode` against the test include URL |
| Activate after writing | `activateByName` or `activateObjects` |

Write the test, syntax-check it, set it with `setObjectSource` (needs a `lock` handle), activate, then
`unitTestRun` - that loop is the red/green cycle against a real system.
