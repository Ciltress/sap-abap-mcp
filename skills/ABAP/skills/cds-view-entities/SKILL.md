...
When an ADT connection is available:

| Step | Tool |
| --- | --- |
| Read a CDS entity's source by name | `readAbapObject` |
| Syntax-check a CDS artefact | `syntaxCheckCdsUrl` (not `syntaxCheckCode`) |
| The annotation vocabulary this system knows | `annotationDefinitions` |
| Underlying table's fields, keys and check tables | `describeAbapTable` |
| Semantic model of a view or entity | `ddicElement` |
| Try the projection against real data | `runQuery`, `tableContents` |
| Activate | `activateByName`, then `inactiveObjects` to confirm nothing is left inactive |

`describeAbapTable` is the quickest way to get a base table's key fields and foreign keys before
modelling associations - the `checkTable` of a field is its association target in all but name.
