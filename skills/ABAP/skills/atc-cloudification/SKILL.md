...
This server exposes the whole ATC workflow, so a cloud-readiness run can be executed rather than only
described:

| Step | Tool |
| --- | --- |
| Read the system's configured variant | `atcCustomizing` - `properties[].systemCheckVariant` |
| Validate a variant name before using it | `atcCheckVariant` |
| Run over an object **or a whole package** | `createAtcRun` |
| Fetch the findings | `atcWorklists` - objects with findings, priority and location |
| Propose and request an exemption | `atcExemptProposal` then `atcRequestExemption` |
| Who can approve it | `atcUsers` |

Take the variant from `atcCustomizing` rather than assuming `DEFAULT_REMOTE` - on a stock on-premise
system it is often plain `DEFAULT`. Exemption reasons (`FPOS`, `OTHR`) require a justification text.
