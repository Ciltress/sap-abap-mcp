...
| Step | Tool |
| --- | --- |
| Read the BO's artefacts by name | `readAbapObject` (behaviour definition, projection, classes) |
| Check a behaviour pool compiles | `syntaxCheckCode` against its `sourceUrl` |
| Inspect a service binding | `bindingDetails` |
| Publish / unpublish a binding | `publishServiceBinding`, `unPublishServiceBinding` |
| Activate the whole BO | `activateObjects`; `inactiveObjects` shows what is still inactive |
| Run the BO's unit tests | `unitTestRun` |
| Underlying tables | `describeAbapTable` |

A RAP BO spans many objects, so activate them together with `activateObjects` rather than one at a
time - a behaviour definition and its pool must activate as a set.
