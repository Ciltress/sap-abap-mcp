...
When an ADT connection is available, the abapGit repositories on the system can be driven directly:

| Step | Tool |
| --- | --- |
| List the linked repositories | `gitRepos` |
| Inspect a remote before linking | `gitExternalRepoInfo`, `remoteRepoInfo` |
| Link a package to a repository | `gitCreateRepo` |
| Pull | `gitPullRepo` |
| See what would be pushed | `stageRepo` then `checkRepo` |
| Push | `pushRepo` |
| Switch branch | `switchRepoBranch` |
| Unlink | `gitUnlinkRepo` |

The transport-vs-git decisions in this skill are unchanged; these tools only remove the SAP GUI step.
Serialized files on disk are still what abaplint reads, so a pull is the bridge between the two.
