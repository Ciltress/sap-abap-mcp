...
| Step | Tool |
| --- | --- |
| Inspect a service binding and its services | `bindingDetails` |
| Publish / unpublish | `publishServiceBinding`, `unPublishServiceBinding` |
| Read the data provider or model provider class | `readAbapObject` |
| Check a redefined method compiles | `syntaxCheckCode` |
| Activate | `activateByName`, `activateObjects` |

For a V2 SEGW service the DPC/MPC extension classes are ordinary classes - `readAbapObject` reads them
by name, including their `includes`.
