...
Exporting to PNG, SVG or PDF needs the draw.io CLI, whose binary name and path differ per platform
(macOS `drawio`, Linux, Windows, WSL2). The commands are in
[`reference/export-cli.md`](reference/export-cli.md) — read it when you actually export, not before.
Delivering the `.drawio` file itself needs none of this.
