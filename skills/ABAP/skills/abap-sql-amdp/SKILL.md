...
When an ADT connection is available, SQL can be tried instead of reasoned about:

| Step | Tool |
| --- | --- |
| Fields, types, keys and check tables of a table | `describeAbapTable` |
| Run an ad-hoc SELECT | `runQuery` |
| Read a table's rows | `tableContents` |
| Check an AMDP class or method compiles | `syntaxCheckCode` |
| Read the AMDP class | `readAbapObject` |

`describeAbapTable` gives the key fields, which is what decides whether `SELECT SINGLE` is legitimate
or an `UP TO 1 ROWS` is required. Its `checkTable` per field is the join target. Constrain every
`runQuery` with a `WHERE` clause - these are production-sized tables.
